...
ggplot(data=contact.F,aes(x=AgeGroup.X,y=AgeGroup.Y)) + geom_count() + xlab("Age group of confirmed case") + ylab("Age group of contact")  + geom_count(data=contact.F[contact.F$case==TRUE,],aes(x=AgeGroup.X,y=AgeGroup.Y),color="blue")

x7 <- glm(data=contact.merge[contact.merge$`Contact type`=="Family member",],case ~ relevel(as.factor(AgeGroup.Y),ref="50-59")*relevel(as.factor(AgeGroup.X),ref="50-59"),family=binomial)
summary(x7)
