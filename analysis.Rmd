...
testsub <- testingU %>% filter(!is.na(PersonalID)) %>% dplyr::select(PersonalID,Gender,Age,Date)
testdate <- testingU %>% dplyr::select(PersonalID,Date) ### need to start from full PCR testing database for this
contacts2 <- left_join(contacts,testdate,by=c("Patient unique ID"="PersonalID"))

contact.merge <- left_join(contacts2,testsub,by=c("Contact ID"="PersonalID"))
